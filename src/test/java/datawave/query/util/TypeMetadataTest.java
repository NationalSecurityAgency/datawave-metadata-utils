...
public class TypeMetadataTest {
    ...
    public void testReadNewSerializedFormatMultipleFieldsAndTypes2() {
        TypeMetadata fromString = new TypeMetadata("dts:[0:generic,1:null];types:[0:datawave.data.type.GeoType,1:datawave.data.type.LcNoDiacriticsType,2:datawave.data.type.NumberType];CITY:[0:1,1:1];CITY_STATE:[0:1,];CODE:[0:1,1:1];CONTINENT:[0:1,1:1];GEO:[0:0,1:0];NUM:[0:2,1:2];STATE:[0:1,]");
        ...
        Set<String> normalizers = fromString.getNormalizerNamesForField("CITY");
        assertEquals(1, normalizers.size());
        assertTrue(normalizers.contains("datawave.data.type.LcNoDiacriticsType"));
    }
